...
namespace ImGui
{
    ...
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Debug/Metrics window. display Dear ImGui internals: draw commands (with individual draw calls and vertices_), window list, basic internal state, etc.
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset  = 1 << 3,   // Back-end Renderer supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+ vertices_) while still using 16-bit indices.
...
//   those fields allow us to render meshes larger than 64K vertices_ while keeping 16-bit indices.
...
struct ImDrawCmd
{
    ...
    unsigned int    VtxOffset;          // 4    // Start offset in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support meshes larger than 64K vertices_ with 16-bit indices.
    ...
    ImDrawCallback  UserCallback;       // 4-8  // If != NULL, call the function instead of rendering the vertices_. clip_rect and texture_id will be set normally.
...
struct ImDrawList
{
    ...
    unsigned int            _VtxCurrentIdx;     // [Internal] Generally == VtxBuffer.Size unless we are past 64K vertices_, in which case this gets reset to 0.
    ...
    // - We render triangles (three vertices_)
